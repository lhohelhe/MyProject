...
<div class="max-w-2xl mx-auto" x-data="materiEditor()">
    <h1 class="mb-8 text-3xl font-black text-black uppercase tracking-wider sm:text-3xl lg:text-4xl font-jakarta">Tambah Materi</h1>
        ...
        <form action="{{ route('materi.store', ['active_menu' => request()->input('active_menu')]) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div x-data="cascadeMateri()">
                <div class="grid grid-cols-1 gap-6 mb-6 md:grid-cols-3">
                    <div>
                        <label class="block mb-2 text-sm font-bold text-black font-jakarta">Buku</label>
                        <select x-model="selectedBuku" @change="fetchBab()" class="w-full px-4 py-3 border-2 border-black rounded-xl font-jakarta focus:outline-none focus:ring-2 focus:ring-[#F4922A] focus:border-transparent" required>
                    ...
                    <div>
                        <label class="block mb-2 text-sm font-bold text-black font-jakarta">Bab</label>
                        <select x-model="selectedBab" @change="fetchSubbab()" class="w-full px-4 py-3 border-2 border-black rounded-xl font-jakarta focus:outline-none focus:ring-2 focus:ring-[#F4922A] focus:border-transparent" required>
                    ...
                    <div>
                        <label class="block mb-2 text-sm font-bold text-black font-jakarta">Subbab</label>
                        <select x-model="selectedSubbab" name="id_subbab" class="w-full px-4 py-3 border-2 border-black rounded-xl font-jakarta focus:outline-none focus:ring-2 focus:ring-[#F4922A] focus:border-transparent" required>
            ...
            <div>
                ...
                <input type="text" id="judul_materi" name="judul_materi" value="{{ old('judul_materi') }}" required class="w-full px-4 py-3 border-2 border-black rounded-xl font-jakarta focus:outline-none focus:ring-2 focus:ring-[#F4922A] focus:border-transparent" />
            </div>

            <!-- PDF Upload (Phase 1) -->
            <div x-show="!extracted" x-cloak style="display:none;">
                <label for="pdf" class="block mb-2 text-sm font-bold text-black font-jakarta">PDF Materi (opsional)</label>
            ...
            <div class="w-72 bg-gray-50 border-l p-4 overflow-y-auto mt-6">
                ...
                <template x-for="term in terms" :key="term.id">
                    <div class="flex items-center justify-between py-1 border-b border-gray-200">
                        <div class="flex-1 cursor-pointer" @click="scrollToTerm(term)"><span x-text="term.text" class="font-bold text-black"></span></div>
            ...
            <div class="flex gap-4 pt-6">
                <button type="submit" class="flex-1 py-3 text-lg font-bold text-white bg-[#F4922A] border-2 border-black shadow-[3px_3px_0px_#000] rounded-xl hover:shadow-none hover:translate-x-[3px] hover:translate-y-[3px] transition-all font-jakarta">Simpan</button>
                <a href="{{ url()->previous() }}" class="flex-1 py-3 text-lg font-bold text-center text-black bg-white border-2 border-black shadow-[2px_2px_0px_#000] rounded-xl hover:shadow-none hover:translate-x-[2px] hover:translate-y-[2px] transition-all font-jakarta">Batal</a>
            </div>
        </form>
    </div>
</div>
